...
| 🩺 **Built-in diagnostics** | `agent-search doctor` — one command shows what works, what doesn't, and how to fix it |
...
After the Skill is installed, the Agent will auto-detect whether `agent-search` CLI is available and install it if needed.

> If you install via `agent-search install`, the skill is registered automatically — no extra steps needed.
</details>
...
Each channel file only checks whether its upstream tool is installed and working (`check()` method for `agent-search doctor`). The actual reading and searching is done by calling the upstream tools directly.
...
| Xiaoyuzhou Podcast | `transcribe.sh` | `bash ~/.agent-search/tools/xiaoyuzhou/transcribe.sh <URL>` |
...
This project was entirely vibe-coded 🎸 There might be rough edges here and there — sorry about that! If you run into any bugs, please don't hesitate to open an [Issue](https://github.com/travis-burmaster/agent-search-tool/issues) and I'll fix it ASAP.
...
[PRs](https://github.com/travis-burmaster/agent-search-tool/pulls) always welcome!
...
Yes! Agent Reach is an installer + configuration tool. Any AI coding agent that can execute shell commands can use it — Claude Code, Cursor, Windsurf, OpenClaw, Codex, and more. Just `pip install agent-search`, run `agent-search install`, and the agent can start using the upstream tools immediately.
</details>
...
For bug reports and feature requests, please use [GitHub Issues](https://github.com/travis-burmaster/agent-search-tool/issues).
