...
Usage:
    ...
    agent-search doctor
    agent-search configure twitter-cookies "auth_token=xxx; ct0=yyy"
    agent-search setup
"""
...
def main():
    ...
    parser.add_argument("--version", action="version", version=f"Agent Search v{__version__}")
    ...
    p_uninstall.add_argument("--keep-config", action="store_true",
                             help="Remove skill files only, keep ~/.agent-search/ config and tokens")
    ...
    if args.command == "version":
        print(f"Agent Search v{__version__}")
        sys.exit(0)
...
def _cmd_install(args):
    ...
    from agent_search.doctor import check_all, format_report
    ...
    config = Config()
    print()
    print("Agent Search Installer")
    print("=" * 40)

    # Ensure tools directory exists (for upstream tool repos)
    tools_dir = os.path.expanduser("~/.agent-search/tools")
    ...
    if env == "local" and needs_cookies and not safe_mode and not dry_run:
        ...
        try:
            from agent_search.cookie_extract import configure_from_browser
    ...
    if env == "server":
        ...
        print("   For Bilibili full access: agent-search configure proxy http://user:pass@ip:port")
    ...
    if not dry_run:
        ...
        if not requested_channels:
            ...
            print("   agent-search install --channels=twitter,weibo,xiaohongshu,...")
            print("   agent-search install --channels=all  (install everything)")

        # Star reminder
        print()
        print("If agent-search-tool helped you, consider giving it a star:")
        print("   https://github.com/travis-burmaster/agent-search-tool")
        print("   Takes one second and helps others find it. Thanks!")
    else:
        print()
...
def _install_skill():
    ...
    for skill_dir in skill_dirs:
        if os.path.isdir(skill_dir):
            target = os.path.join(skill_dir, "agent-search")
    ...
    if not installed:
        ...
        target = os.path.expanduser("~/.agents/skills/agent-search")
...
def _uninstall_skill():
    ...
    skill_dirs = [
        ("~/.openclaw/skills/agent-search", "OpenClaw"),
        ("~/.claude/skills/agent-search", "Claude Code"),
        ("~/.agents/skills/agent-search", "Agent"),
    ]
    ...
    if openclaw_home:
        skill_dirs.insert(
            0,
            (os.path.join(openclaw_home, ".openclaw", "skills", "agent-search"), "OpenClaw"),
        )
...
def _cmd_format(args):
    ...
    if args.platform == "xhs":
        from agent_search.channels.xiaohongshu import format_xhs_result
...
def _install_xiaoyuzhou_deps():
    ...
    tools_dir = os.path.expanduser("~/.agent-search/tools/xiaoyuzhou")
    ...
    if has_key:
        ...
        print("     Then run: agent-search configure groq-key gsk_xxxxx")
...
def _install_weibo_deps():
    """Install Weibo MCP server (upstream fork with visitor passport auth)."""
    import shutil
    import subprocess
...
def _cmd_configure(args):
    ...
    if args.from_browser:
        ...
        print()
        if found_any:
            print("✅ Cookies configured! Run `agent-search doctor` to see updated status.")
        else:
    ...
    # ── Manual configure ──
    if not args.key:
        print("Usage: agent-search configure <key> <value>")
        print("   or: agent-search configure --from-browser chrome")
        return
    ...
    if args.key == "proxy":
        ...
        if auth_token and ct0:
            ...
            print("   1. agent-search configure twitter-cookies AUTH_TOKEN CT0")
            print('   2. agent-search configure twitter-cookies "auth_token=xxx; ct0=yyy; ..."')
...
def _configure_xhs_cookies(value):
    ...
    if not value:
        ...
        print("   Usage: agent-search configure xhs-cookies '<cookie JSON or header string>'")
        return
    ...
    if not docker:
        ...
        cookie_path = os.path.expanduser("~/.agent-search/xhs-cookies.json")
...
def _cmd_uninstall(args):
    ...
    print()
    print("Agent Search Uninstaller")
    print("=" * 40)
    ...
    # ── 1. Config directory (~/.agent-search/) ──
    config_dir = os.path.expanduser("~/.agent-search")
    if not keep_config:
...
def _cmd_setup():
    ...
    print("运行 agent-search doctor 查看完整状态")
    print()
...
def _cmd_check_update():
    ...
    release_url = "https://api.github.com/repos/travis-burmaster/agent-search-tool/releases/latest"
    commit_url = "https://api.github.com/repos/travis-burmaster/agent-search-tool/commits/main"
    ...
    if resp.status_code == 200:
        ...
        if latest and latest != __version__:
            ...
            print()
            print("更新命令:")
            print("  pip install --upgrade https://github.com/Panniantong/agent-search/archive/main.zip")
...
def _cmd_watch():
    ...
    """
    from agent_search.config import Config
    from agent_search.doctor import check_all
    from agent_search import __version__
    ...
    resp, err, _attempts = _github_get_with_retry(
        "https://api.github.com/repos/travis-burmaster/agent-search-tool/releases/latest",
        timeout=10,
        retries=2,
    )
    ...
    if update_available:
        ...
        if release_body:
            for line in release_body.strip().split("\n")[:10]:
                print(f"    {line}")
        print(f"  更新: pip install --upgrade https://github.com/Panniantong/agent-search/archive/main.zip")
