...
from agent_search.channels import get_all_channels
...
def test_youtube_warns_when_node_only_and_no_config(monkeypatch, tmp_path):
    ...
    from agent_search.channels.youtube import YouTubeChannel
...
def test_youtube_warns_with_windows_specific_fix_command(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("shutil.which", fake_which)
    monkeypatch.setattr("agent_search.utils.paths.sys.platform", "win32")
...
def test_douyin_check_does_not_call_with_invalid_url(monkeypatch, tmp_path):
    ...
    from agent_search.channels.douyin import DouyinChannel
